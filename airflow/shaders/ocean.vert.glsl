...
void main() {
  ...
  for (int i = 0; i < 8; i++) {
    ...
    float rippleSpeed = mix(9.4, 4.2, viscMix);
    float spatialFreq = mix(6.8, 5.0, viscMix * 0.9);
    float radialDecay = mix(1.18, 1.86, viscMix);
    float temporalDecay = mix(0.98, 2.35, viscMix);
    float rampTime = mix(0.14, 0.22, viscMix);
    float singleReflectionGain = u_wallReflectivity * 0.82;
    float cornerReflectionGain = u_wallReflectivity * u_wallReflectivity * 0.68;
